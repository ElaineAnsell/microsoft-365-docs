...
Note that assessment results reflect a point-in-time check and changes in Intune, Azure AD and M365 policies after running this check may shift settings that are in Ready state to Not ready.  Prior to any policy changes, review the conditions below affecting a specific setting to ensure the change will not cause errors in Microsoft Management Desktop operations.
